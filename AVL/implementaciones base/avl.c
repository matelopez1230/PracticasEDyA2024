...
static AVL_Nodo* avl_nodo_insertar(AVL_Nodo* raiz, void* dato,
  ...
  if (raiz == NULL) // insertamos el nuevo elemento
    ...
    raiz->der = avl_nodo_insertar(raiz->izq, dato, copia, comp);
    if (avl_nodo_factor_balance(raiz) == -2) {
...
static AVL_Nodo *avl_nodo_minimo(AVL_nodo* nodo) {
  while (nodo->izq != NULL) {
    nodo = nodo->izq;
  }
  return nodo;
...
static AVL_Nodo* avl_nodo_eliminar(AVL_Nodo* raiz, void* dato, FuncionComparadora comp, FuncionDestructora destr) {
  ...
  if (comp(dato, raiz->dato) < 0) {
    ...
    if (raiz->izq == NULL || raiz->der == NULL) {
      ...
      AVL_Nodo* temp = avl_nodo_minimo(raiz->der);
      void* dato_temp = raiz->dato;
      raiz->dato = temp->dato;
      temp->dato = dato_temp;
      raiz->der = avl_nodo_eliminar(raiz->der, dato_temp, comp, destr);
    }
  }
  ...
  if (balance > 1 && avl_nodo_factor_balance(raiz->der) >= 0)
  ...
  if (balance < -1 && avl_nodo_factor_balance(raiz->izq) <= 0)
