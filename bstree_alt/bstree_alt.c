...
static BST_Nodo *bstree_nodo_insertar(BST_Nodo *raiz, void *dato, FuncionCopiadora copia, FuncionComparadora comp) {
    if (raiz == NULL) { // insertar el dato en un nuevo nodo
        BST_Nodo *nuevoNodo = malloc(sizeof(struct BSTree));
...
void bstree_nodo_recorrer(BST_Nodo *raiz, BSTreeRecorrido orden,FuncionVisitanteExtra visita, void *extra) {
    if (raiz != NULL) {
